...
## ติดตั้งเป็นแอป
- หน้า **/install** บอกขั้นตอนทั้ง iPhone และ Android (เลือกให้ตามเครื่อง · บังคับได้ด้วย `?os=ios` / `?os=android`) — แบบ "ดูรูปแล้วกดตาม": ไอคอนของสิ่งที่ต้องกด + หน้าจอจำลองวงส้ม + กล่อง "ไม่เห็นเมนู?" — HR ส่งลิงก์ `app.amgovenger.com/install` ให้พนักงานใน LINE ได้เลย
