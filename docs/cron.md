...
**ทำอะไร:** ลบรูปเซลฟี่ตอนเช็คอิน + รูปหลักฐานส่งของ + รูปสต็อก/หน้าร้าน ที่เก่ากว่า 60 วัน
(รูปสต็อกแยกค่าคงที่ `STOCK_RETENTION_DAYS` ไว้ — อยากไล่ดูของหายย้อนไกลกว่านั้นแก้เลขเดียว)
...
## ตาข่ายพื้นที่ (7 ก.ย. 69) — ใน cleanup-photos เดิม
- หลังลบตามกฎ 60 วัน จะวัดพื้นที่ทั้งโปรเจกต์ (`storage_usage()`) เทียบโควตา `app_config.storage_quota_mb` (Free = 1024)
- ใช้เกิน **85%** → ลบรูป "เก่าสุดทั้งระบบ" (เทียบวันเก่าสุดของเช็คอิน/ส่งของ/สต็อก) ทีละ 100 แถว สูงสุด 6 รอบ/คืน จนเหลือ ≤ **75%** · คืนถัดไปไล่ต่อ
- ไม่แตะ bucket โลโก้ · avatar · รูปสินค้า SRP · สลิป — ถ้าพวกนี้ทำให้เกินเอง ต้องอัปเกรดแพลน (response `cap.stillOver`)
- ดูสถานะ: หน้ารายงานรูปสต็อก การ์ด "พื้นที่เก็บรูป" (admin กดแก้โควตาได้) หรือ `GET /api/storage/usage`
